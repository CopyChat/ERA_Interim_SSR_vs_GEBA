...
file=ERA.ssrd.mon.1983-2005.nc
PWD=$(pwd)
...
{
    resolution=0.75
    step=$(echo $resolution | awk '{print $1/2}')
    ...
    do
	    ...
	    lat1=$(awk 'NR=='$i'{print $3-0.375}' $GEBA)
	    lat2=$(awk 'NR=='$i'{print $3+0.375}' $GEBA)

	    lon1=$(awk 'NR=='$i'{print $4-0.375}' $GEBA)
	    lon2=$(awk 'NR=='$i'{print $4+0.375}' $GEBA)
...
#    PARAMETERS:  output, var name,
#       RETURNS:  cmsaf.rsds.csv
#-------------------------------------------------------------------------------

function ERAprepare()
{
    output=era_in.rsds.csv
    cd $DIR/MONTH
    ...
    do
        ...
        for m in 01 02 03 04 05 06 07 08 09 10 11 12
        do
            value=$(getvalue_netCDF.py ssrd $year${m}_$sta.nc)
            echo -n $value, >> $output
...
#### change '/' to ',' for the station names
